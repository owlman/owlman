# 关于我

[![GitHub stars](https://img.shields.io/github/stars/owlman/CS_StudyNotes?style=flat-square)](https://github.com/owlman/CS_StudyNotes/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/owlman/CS_StudyNotes?style=flat-square)](https://github.com/owlman/CS_StudyNotes/network/members)
[![License](https://img.shields.io/badge/license-CC%20BY--NC--SA%204.0-blue?style=flat-square)](https://github.com/owlman/CS_StudyNotes/blob/main/LICENSE)

> **「我更愿意慢，但要站得住、可追溯、可重复引用。」**

我是一名技术写作者与计算机系统学习者，专注长期积累与可复用的技术判断。

TL;DR：这里整理计算机基础、工具链取舍、写作方法与实验记录，目标是让内容在三年、五年后依然有参考价值。

## 我在做什么

1. **计算机科学领域的长期笔记**
   - 算法 / 操作系统 / 编译原理等基础路线与笔记
   - 语言学习体会：Python、JavaScript、C++ 等

2. **开发工具与工程实践**
   - 前端与后端框架：Node.js、Vue.js、uni-app 的实践总结
   - 编辑器与 IDE：NVim、VS Code、OpenCode 使用经验
   - 运维与部署：Git、Docker、Kubernetes 等工具的实战记录

3. **技术写作与可复现实验**
   - 使用 Obsidian / Markdown / LaTeX 作为写作工具
   - 记录完整实验过程（环境、命令、结果），便于复现
   - 把学习笔记打磨成有判断力的文章

## 我不做什么

- 不产出标题党或情绪化内容
- 不把 AI 当作替代思考的工具
- 不追逐短期热点或流量红利
- 不假装无所不知

## 适合谁关注我

- 想知道「为什么」而不仅是「怎么做」的读者
- 需要完整思路、权衡过程与失败记录的人
- 注重长期积累与系统学习而非速成的学习者

## 联系与延伸

- 邮箱：[jie.owl2008@gmail.com](mailto:jie.owl2008@gmail.com)
- 博客：[https://www.cnblogs.com/owlman](https://www.cnblogs.com/owlman)
- GitHub：[https://github.com/owlman](https://github.com/owlman)
- 讨论与反馈：请在相关仓库提交 [GitHub Issues](https://github.com/owlman/CS_StudyNotes/issues)

## 已出版作品

**原创**

- 《JavaScript 全栈开发》— 2021年6月出版，这是一本覆盖前端到后端的全栈开发指南
- 《Vue.js 全平台前端实战》— 2022年5月出版，这是一本基于 Vue.js 的跨平台开发实践
- 《Node.js 后端全程实战》— 2023年4月出版，这是一本从入门到部署的 Node.js 后端教程

**翻译**

- 《Python 算法教程》— 用 Python 语言讲解算法与数据结构的基础知识
- 《C++ 程序设计》（第 3 版）— C++ 语言程序设计的系统教材
- 《JavaScript 面向对象编程指南》（第 1、2 版）— 深入讲解 JavaScript 面向对象编程范式
- 《测试驱动的 JavaScript 开发》— TDD 理念在 JavaScript 开发中的应用
- 《元素模式》— 软件设计模式与最佳实践
- 《NLTK 基础教程》— 自然语言处理工具 NLTK 的入门指南
- 《Git 学习指南》— 分布式版本控制系统 Git 的实践教程
- 《Visual C++ 并行编程实战》— 并行计算在 Visual C++ 中的实战应用
- 《HTML5 和 CSS3 快速参考》— HTML5 与 CSS3 快速查阅手册

## 这里的主要内容（主要仓库）

- [CS_StudyNotes](https://github.com/owlman/CS_StudyNotes) — 计算机科学与工程学习笔记（持续整理中）
- [HTML_CSS_in_action](https://github.com/owlman/HTML_CSS_in_action) — 一本面向前端初学者的网页设计教材，通过 5 个企业级项目实践，循序渐进地教授 HTML5 + CSS3 + Bootstrap 网页设计技术。
- [markdown_guide](https://github.com/owlman/markdown_guide) — 使用 Markdown 写作与知识表达的实践总结
- 实验/环境仓库 — 用于验证、复现技术方案（见各仓库 README）

## 贡献与许可

- 欢迎通过 Issues 提问或提交 PR（请先阅读仓库中的 CONTRIBUTING 或相应说明）。
- 许可证：参见仓库根目录的 LICENSE 文件。
